...
class ItemCatalogo(models.Model):
    ...
    # Campos novos para compatibilidade com o frontend
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            slug = base
            n = 1
            while ItemCatalogo.objects.filter(slug=slug).exclude(pk=self.pk).exists():
                slug = f"{base}-{n}"
                n += 1
            self.slug = slug
...
class Carrinho(models.Model):
    ...
    def total(self):
        return sum(
            (item.subtotal for item in self.itens.select_related("item_catalogo")),
            Decimal("0.00"),
        )
...
class ItemCarrinho(models.Model):
    ...
    item_catalogo = models.ForeignKey(
        ItemCatalogo, on_delete=models.PROTECT, related_name="itens_em_carrinho"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["carrinho", "item_catalogo"], name="uq_item_por_carrinho"
            ),
        ]
